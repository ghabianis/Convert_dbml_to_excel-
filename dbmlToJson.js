    ...
    if (error) {
        ...
        function createTableRelationsArray(tableRelations) {
            ...
            tableRelations.forEach((relation) => {
                ...
                relationsDataSet.push({ 'PrimaryTable': relation[0].tableName, 'SecondaryTable': relation[1].tableName, "field": relation[1].fieldNames[0], "relation": relation[0].relation[0] === "1" ? "one-to-many" : "many-to-one" })
                const relationType = relation[0].relation === "1" ? "one-to-many" : "many-to-one";
...
function deleteFirstRowOfExcelSheets(filePath) {
    ...
    sheetNames.forEach(sheetName => {
        ...
        const filteredData = data.filter(row => {
          return !(row.includes("createdAt") || row.includes("deletedAt") || row.includes("id") || row.includes("updatedAt"));
        });
